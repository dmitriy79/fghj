...
const char * const BITCOIN_CONF_FILENAME = "hfcc.conf";
const char * const BITCOIN_PID_FILENAME = "hfccd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "helpforcancer" is a composite category enabling all Helpforcancer-related debug output
                if(ptrCategory->count(std::string("hfcc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HelpforcancerCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HelpforcancerCore
    // Mac: ~/Library/Application Support/HelpforcancerCore
    // Unix: ~/.helpforcancercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hfcc";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Hfcc";
#else
    // Unix
    return pathRet / ".hfcc";
#endif
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty helpforcancer.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override helpforcancer.conf
