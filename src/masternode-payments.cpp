...
*
*   Why is this needed?
*   - In Helpforcancer some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Helpforcancer specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f HELPFORCANCER\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
