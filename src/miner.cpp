...
{
    LogPrintf("HelpforcancerMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("helpforcancer-miner");
    ...
    while (true) {
        try {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HelpforcancerMiner -- Failed to find a coinstake\n");
                MilliSleep(5000);
                continue;
            }
            auto pblock = std::make_shared<CBlock>(pblocktemplate->block);
            IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);
            LogPrintf("HelpforcancerMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        {
            LogPrintf("HelpforcancerMiner -- terminated\n");
            throw;
        }
        catch (const std::runtime_error &e)
        {
            LogPrintf("HelpforcancerMiner -- runtime error: %s\n", e.what());
//            return;
        }
    }
}
